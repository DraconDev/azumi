...
class Azumi {
    ...
    executePrediction(scopeElement, prediction) {
        ...
        try {
            // Handle signed state: "{json}|{timestamp}|{signature}"
            // Must strip both the signature (last pipe) AND timestamp (second to last pipe)
            let jsonStr = scopeAttr;
            if (scopeAttr.includes("|")) {
                const lastPipe = scopeAttr.lastIndexOf("|");
                let stripped = scopeAttr.substring(0, lastPipe); // Remove signature
                const secondLastPipe = stripped.lastIndexOf("|");
                if (secondLastPipe !== -1) {
                    stripped = stripped.substring(0, secondLastPipe); // Remove timestamp
                }
                jsonStr = stripped;
            }
